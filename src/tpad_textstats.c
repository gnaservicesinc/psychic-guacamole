...
//	ui_unity_set_count();
	...
	gtk_box_append(GTK_BOX(mainbox), wordCountbox);
    gtk_box_append(GTK_BOX(mainbox), charCountbox);
    gtk_box_append(GTK_BOX(mainbox), lineCountbox);



	gtk_box_append(GTK_BOX(wordCountbox), GTK_WIDGET(gtk_label_new(_WORD_COUNT_HEADING)));
	gtk_box_append(GTK_BOX(wordCountbox), GTK_WIDGET(vseparator[1]));
	gtk_box_append(GTK_BOX(wordCountbox), GTK_WIDGET(gtk_label_new(cword_count)));

	gtk_box_append(GTK_BOX(charCountbox), GTK_WIDGET(gtk_label_new(_CHAR_COUNT_HEADING)));
	gtk_box_append(GTK_BOX(charCountbox), GTK_WIDGET(gtk_label_new(cchar_count)));
	gtk_box_append(GTK_BOX(charCountbox), GTK_WIDGET(vseparator[2]));

	gtk_box_append(GTK_BOX(lineCountbox), GTK_WIDGET(gtk_label_new(_LINE_COUNT_HEADING)));
	gtk_box_append(GTK_BOX(lineCountbox), GTK_WIDGET(gtk_label_new(cline_count)));
	gtk_box_append(GTK_BOX(lineCountbox), GTK_WIDGET(vseparator[3]));
